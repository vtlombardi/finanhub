...
async function main() {
  console.log("🚀 Iniciando Seeding Controlado...");

  // 0. Proteção: Verificar se já existe seed
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    const listing = await prisma.listing.create({
      data: {
        ...
        slug: `oportunidade-teste-${i}-${Date.now()}`,
        ...
        // Adicionando alguns atributos simulados para a busca/filtros
        employeesCount: `${Math.floor(Math.random() * 50) + 5}`,
        revenue: `R$ ${ (price * 0.2).toLocaleString('pt-BR') }/ano`,
        ebitda: `R$ ${ (price * 0.05).toLocaleString('pt-BR') }/ano`
      }
    });
    createdIds.push(listing.id);
    
    // Simular IA Insight para filtros de score
